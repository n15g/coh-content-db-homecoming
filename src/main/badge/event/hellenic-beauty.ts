import { BadgeData } from 'coh-content-db'

export const HellenicBeauty: BadgeData = {
  type: 'event',
  key: 'hellenic-beauty',
  setTitleId: [2243],
  name: 'Hellenic Beauty',
  morality: 'all',
  badgeText: 'You have let beauty prevail by letting Camilla and Claudia go unscathed while defeating the ugly brute Cornutus.',
  acquisition: 'In the Summer Blockbuster, defeat Cornutus, but leave both Camilla and Claudia alive.',
  notes: `It is not necessary to refrain from damaging the two angels, just don't kill them.`,
  links: [
    { title: 'Hellenic Beauty Badge', href: 'https://homecoming.wiki/wiki/Hellenic_Beauty_Badge' },
  ],
  icon: 'https://n15g.github.io/coh-content-db-homecoming/images/badges/event/hellenic-beauty.png',
}
