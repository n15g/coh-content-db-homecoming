import { BadgeData } from 'coh-content-db'

export const HammerDown: BadgeData = {
  type: 'defeat',
  key: 'hammer-down',
  setTitleId: [304],
  name: 'Hammer Down',
  releaseDate: '2012-11-30',
  morality: 'villainous',
  badgeText: `That'll teach Scrapyard to mess with you.`,
  acquisition: 'Defeat the Ghost of Scrapyard, a monster that spawns in Sharkhead Isle.',
  links: [
    { title: 'Hammer Down Badge', href: 'https://homecoming.wiki/wiki/Hammer_Down_Badge' },
  ],
  icon: 'https://n15g.github.io/coh-content-db-homecoming/images/badges/defeat/scrapyarder.png',
}
