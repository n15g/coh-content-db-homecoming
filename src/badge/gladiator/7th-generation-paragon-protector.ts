import {ALIGNMENT_ANY, BadgeType, IBadgeData} from "coh-content-db";

export const _7thGenerationParagonProtector: IBadgeData = {
    type: BadgeType.GLADIATOR,
    key: "7th-generation-paragon-protector",
    setTitleId: 501,
    names: [
        {value: "7th Generation Paragon Protector"}
    ],
    alignment: ALIGNMENT_ANY,
    badgeText: [
        {value: "This will be a powerful gladiator in the Arena."}
    ],
    acquisition: "Earn the [badge:infiltrator] Badge (Crey)",
    links: [
        {title: "7th Generation Paragon Protector Badge", href: "https://paragonwiki.com/wiki/7th_Generation_Paragon_Protector_Badge"}
    ],
    icons: [
        {value: "https://n15g.github.io/coh-content-db-homecoming/images/badges/gladiator/gladiator.png"}
    ],
};
